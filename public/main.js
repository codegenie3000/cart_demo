...
var overlay = function () {
    var fadeIn = function fadeIn() {
        (0, _jquery2.default)('.custom-overlay').fadeIn('fast', function () {
            // animation complete
        });
    };

    var fadeOut = function fadeOut() {
        (0, _jquery2.default)('.custom-overlay').fadeOut('fast', function () {
            // animation complete
        });
    };
...
var features = {
    hasValidEmail: function hasValidEmail(formInput) {
...
var cart = {
    changeQty: function changeQty() {
        (0, _jquery2.default)('#cart').find('select.change-qty').change(function (e) {
            e.preventDefault();
            ...
            _jquery2.default.ajax('/cart/change_qty', {
    ...
    checkout: function checkout() {
        (0, _jquery2.default)('#cart').find('#checkOut').click(function (e) {
...
var billingFunc = {
    ...
    fieldsCompleteCheck: function fieldsCompleteCheck() {
        var $button = billingFunc.$form.find(':button');
        (0, _jquery2.default)('.required').on('input propertychange paste', function () {
            (0, _jquery2.default)('.required').each(function () {
                if (!(0, _jquery2.default)(this).val()) {
                    $button.prop('disabled', 'disabled');
                    return false;
        ...
        billingFunc.$form.find('#email').on('input propertychange paste', function () {
            if (billingFunc.checkEmailAgain) {
                var emailAddress = (0, _jquery2.default)(this).val();
                var emailRegex = /[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/;

                if (emailRegex.test(emailAddress)) {
                    var formGroup = (0, _jquery2.default)('#checkout01-form').find('#email');
    ...
    submitForm: function submitForm() {
        ...
        billingFunc.$form.submit(function (e) {
            ...
            if (features.hasValidEmail(emailAddress)) {
                (0, _jquery2.default)('button, input[type="button"]').blur();
                ...
                // display the spinner
                _jquery2.default.ajax('/cart/billing/submit', {
                    method: 'POST',
...
var shippingFunc = {
    init: function init() {
        shippingFunc.$checkout02 = (0, _jquery2.default)('.step02');
        shippingFunc.$form = shippingFunc.$checkout02.find('#checkout02-form');
        this.checkFieldsCompleteOnLoad();
    },
    checkFieldsCompleteOnLoad: function checkFieldsCompleteOnLoad() {
    ...
    submitForm: function submitForm() {
        ...
        shippingFunc.$form.submit(function (e) {
            e.preventDefault();
            var formObject = function formObject() {
            ...
            _jquery2.default.ajax('/cart/shipping/submit', {
                method: 'POST',
                data: JSON.stringify(formObject()),
                dataType: 'text',
                contentType: 'application/json',
...
var paymentFunc = {
    init: function init() {
        ...
        if ((0, _jquery2.default)('#card-element')[0]) {
            paymentFunc.card.mount('#card-element');

            var $button = (0, _jquery2.default)('.card-entry').find(':button').last();
    ...
    submitListener: function submitListener() {
    ...
    tokenHandler: function tokenHandler(token) {
        _jquery2.default.ajax('/cart/stripeSubmit', {
            method: 'POST',
...
var modalAjaxHandler = {
    ...
    success: function success(data) {
        if (data.allClear !== 'true') {
            (0, _jquery2.default)('#modal-message').text(data.modalMessage);
            (0, _jquery2.default)('#button-message').text(data.buttonMessage);
            (0, _jquery2.default)('a.modal-url').attr('href', data.buttonURL);
            (0, _jquery2.default)('#info-missing-modal').modal({
                backdrop: 'static'
            });
        }
    }
...
(0, _jquery2.default)(function () {
    ...
    if (document.getElementById('checkout') && document.getElementsByClassName('step01')[0]) {
        ...
        _jquery2.default.ajax('/cart/billing/checkIfReady', modalAjaxHandler);
    ...
    if (document.getElementById('checkout') && document.getElementsByClassName('step02')[0]) {
        ...
        _jquery2.default.ajax('/cart/shipping/checkIfReady', modalAjaxHandler);
        shippingFunc.init();
    ...
    if (document.getElementById('checkout') && document.getElementsByClassName('step03')[0]) {
        _jquery2.default.ajax('/cart/confirmation/checkIfReady', modalAjaxHandler);
    }
